...
const isValidCssColor = (color: string): boolean => {
  // Allow hex colors, rgb/rgba/hsl/hsla functions, and CSS color keywords
  const validColorPattern = /^(#[0-9A-Fa-f]{3,8}|rgb\([^)]+\)|rgba\([^)]+\)|hsl\([^)]+\)|hsla\([^)]+\)|[a-zA-Z]+)$/;
  return validColorPattern.test(color.trim());
};

// Sanitizes a CSS property key to only allow alphanumeric and hyphens
...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Build CSS safely by validating all dynamic values
